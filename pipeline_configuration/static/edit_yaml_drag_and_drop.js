...
addNewColumnButton.addEventListener('click', function(event) {
  ...
  var newColumn = createNewSwimLane(columnName);
  document.querySelector(".lanes").appendChild(newColumn);

  resetSwimLaneListeners();
  ...
  pipelineYaml.addEventListener("click", (e) => {
    ...
    fetch(getSinglePipelineDataURL, {
      ...
      response.json().then(data => {
        ...
        var specDetails = data.spec_details;
        ...
        for (var i = 0; i < logicBlocks.length; i++) {
          ...
          document.querySelector(".lanes").appendChild(swimLane);
          resetSwimLaneListeners();
        }
      });
    });
...
function createNewSwimLane(columnName) {
...
function resetSwimLaneListeners() {
